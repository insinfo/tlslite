...
void main() {
  ...
  const rfcVectors = <_Ed448Vector>[
    ...
    _Ed448Vector(
      ...
      ''',
      messageHex: '64a65f3cdedcdd66811e2915',
      signatureHex: '''
    ...
    _Ed448Vector(
      ...
      ''',
      messageHex: '64a65f3cdedcdd66811e2915e7',
      signatureHex: '''
  ...
  group('RFC 8032 Ed448 vectors', () {
    for (final vector in rfcVectors) {
      test(vector.name, () {
        final seed = hexToBytes(vector.seedHex);
        final message = hexToBytes(vector.messageHex);
        final expectedPublic = hexToBytes(vector.publicHex);
        final expectedSignature = hexToBytes(vector.signatureHex);
        final context =
            vector.contextHex.isEmpty ? null : hexToBytes(vector.contextHex);

        final privateKey = ed448.Ed448PrivateKeyImpl.fromSeed(seed);
        expect(
          privateKey.publicKeyBytes,
          equals(expectedPublic),
          reason: 'Public key mismatch for ${vector.name}',
        );

        final signature = privateKey.sign(message, context: context);
        expect(
          signature,
          equals(expectedSignature),
          reason: 'Signature mismatch for ${vector.name}',
        );

        final publicKey = ed448.Ed448PublicKeyImpl(expectedPublic);
        expect(
          publicKey.verify(
            message,
            expectedSignature,
          ...
          isTrue,
          reason: 'Verification failed for ${vector.name}',
        );
      });
    }
  });
